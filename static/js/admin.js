...
;$(function($){
	...
	$('#iconStr').live('click', function(){
		var self = $(this),
			dtitle = '选择图标',
			did = 20151245,
			duri = '/index.php?g=admin&m=menu&a=icon'
		$.dialog({
			id:did,
			title:dtitle,
			width:'auto',
			...
			lock:true,
			ok:function(){
				var info_form = this.dom.content.find('#icon_form');
				if(info_form[0] != undefined){
					var classname=info_form.find("#iconid").val();
					$("#icon_info").attr("class",classname);
		...
		$.get(duri, function(result){
			$.dialog.get(did).content(result);
		});
		return false;
	});
	$(".dialog_content li a").live('click',function(){
		$(this).addClass('on').parent().siblings("li").find("a").removeClass('on');
		$("#iconid").val($(this).attr("class"));
	})
	//附件预览
