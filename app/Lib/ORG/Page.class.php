...
class Page {
    ...
    public function fshow() {
        ...
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<a href='".str_replace('__PAGE__',$upRow,$url)."' class='current_page_prev'><上一页</a>";
        }else{
            $upPage     =   '';
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<a href='".str_replace('__PAGE__',$downRow,$url)."' class='current_page_next'>".$this->config['next']."</a>";
        }else{
            $downPage   =   '';
        }
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a href='".str_replace('__PAGE__',1,$url)."' >";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
        ...
        if ($this->totalPages != 1) {
            ...
            for ($page = $start; $page <= $end; $page++) {
                if ($page != $this->nowPage) {
                    $linkPage .= " <a href='".str_replace('__PAGE__',$page,$url)."'></a>";
                } else {
                    $linkPage .= " <span class='current_page' style='line-height: 30px;'>".$page."/".$end."</span>";
                }
            }
