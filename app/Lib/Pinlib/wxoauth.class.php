...
class wxoauth {
    ...
    public function __construct($type) {
		...
		$setting=$wxpay['config'];
        $this->_setting = unserialize($setting);
        //导入接口文件
        include_once LIB_PATH . 'Pinlib/' . $type . '/' . $type . '.php';
     ...
     * 注册
     */
    public function callbackbinduser($request_args) {
        
        $user = $this->_om->getUserInfo($request_args);
		$checkuser= M('user')->field('id,wechatid')->where("wechatid='".$user['keyid']."'")->find();
       if($checkuser){
			$date=time();
			//$result['wechatid'] = $user['keyid'];//微信id
			//$result['username'] = $user['keyid'];//用户名
			$result['nickname'] = $user['keyname'];//昵称
			$data=unserialize($user['bind_info']);
			$result['province'] = $data['province'];
			$result['city'] = $data['city'];
			$result['gender'] = $data['gender'];
			$userid= M('user')->where("id='".$checkuser['id']."'")->save($result);
			//$result['headerimg'] = $user['headimgurl'];//头像
			$user= M('user')->field('id,username,wechatid')->where("wechatid='".trim($user['keyid'])."'")->find();
			 	if($user)
			 	{
			 		$data['last_time']=time();
